...
0x4f,0x52,0x55,0x58,0x5a,0x5d,0x61,0x64,0x67,0x6a,0x6d,0x70,0x73,0x76,0x79,0x7c   
 ...
 u16 au16BabySong[] = {C4, C4, G4, G4, A4, A4, G4, F4, F4, E4, E4, D4, D4, C4, NN, NN};
 u16 au16NoteDuration[] = {N4, N4, N4, N4, N4, N4, N2, N4, N4, N4, N4, N4, N4, N2, N1, N1};
 static u8 u8index=0;
 static u16 u16NoteCounter=0;
/*--------------------------------------------------------------------------------------------------------------------*/
...
void UserAppInitialize(void)
{
 ...
 InterruptTimerXus(au16NoteDuration[u8index], 1);
...
void UserAppRun(void)
{
    ...
    u16NoteCounter += 1;     // keep track of length each note takes
    
    if(u16NoteCounter >= au16BabySong[u8index])  // if the time of one note has passed
    {
        u8index += 1;                           //increment the index to get to the next note
        u16NoteCounter = 0;                     // reset the note time keeper
        
        if(u8index > 28)                        //making sure array doesn't go out of bounds
        {
            u8index = 0;
        }
